...
typedef struct
{
    ...
    TEMCYQUEUE      sSendEmcyQueue;  
    TEMCYQUEUE      sEmptyEmcyQueue;  
    TEMCYMESSAGE    asEmcy[(0x1)];  
    ...
    void PutInEmptyEmcyQueue(TEMCYMESSAGE  * pEmcy);
    TEMCYMESSAGE  * GetOutOfEmptyEmcyQueue(void);
    void PutInSendEmcyQueue(TEMCYMESSAGE  * pEmcy);
    TEMCYMESSAGE  * GetOutOfSendEmcyQueue(void);
    void EMCY_Init(void);
    unsigned char EMCY_IsQueueEmpty(void);
    void EMCY_ContinueInd(TMBX  * pMbx);
    TEMCYMESSAGE  * EMCY_GetEmcyBuffer(void);
    unsigned char EMCY_SendEmergency(TEMCYMESSAGE  *pEmcy);


 
#line 47 "etherCAT\\emcy.c"
...
const unsigned short  acEmcyCoeHeader[4] = {2 + sizeof(TEMCY), 0, (3<<8), (0x01<<12)};
...
void PutInEmptyEmcyQueue(TEMCYMESSAGE  * pEmcy)
{
    unsigned short lastInQueue = sEmptyEmcyQueue.LastInQueue+1;
...
TEMCYMESSAGE  * GetOutOfEmptyEmcyQueue(void)
{
    ...
    {
        unsigned short firstInQueue = sEmptyEmcyQueue.FirstInQueue;
...
TEMCYMESSAGE  * GetOutOfSendEmcyQueue(void)
{
    ...
    {
        unsigned short firstInQueue = sSendEmcyQueue.FirstInQueue;
...
void EMCY_Init(void)
{
    ...
    sSendEmcyQueue.MaxQueueSize                    = 0x1+1;
    for (i = 0; i < 0x1; i++)
    {
    ...
    sEmptyEmcyQueue.MaxQueueSize                    = 0x1+1;
    for (i = 0; i < 0x1; i++)
    {
...
TEMCYMESSAGE  * EMCY_GetEmcyBuffer(void)
{
    TEMCYMESSAGE * pEmcy = GetOutOfEmptyEmcyQueue();
...
unsigned char EMCY_SendEmergency( TEMCYMESSAGE  *pEmcy )
{
    ...
    if ( bSendMbxIsFull || ((nAlStatus & ((unsigned char) 0x0F)) == ((unsigned char) 0x01))
    ...
    psWriteMbx = (TMBX  *) malloc(((sizeof(acEmcyCoeHeader)+sizeof(TEMCYMESSAGE))));
    ...
    memcpy(psWriteMbx, acEmcyCoeHeader, sizeof(acEmcyCoeHeader));
    memcpy(&psWriteMbx->Data[1], pEmcy, sizeof(TEMCYMESSAGE));
 

    
    
    if (MBX_MailboxSendReq(psWriteMbx, ((unsigned char) 0x0001)) == 0)
    {
