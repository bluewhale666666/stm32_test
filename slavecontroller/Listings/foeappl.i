...
#line 59 "etherCAT\\foeappl.h"
    ...
    unsigned short     FOE_Read(unsigned short  * pName, unsigned short nameSize, unsigned short  * pData, unsigned int password);
    unsigned short     FOE_Write(unsigned short  * pName, unsigned short nameSize, unsigned int password);
    unsigned short     FOE_Data(unsigned short  * pData, unsigned short Size);
    unsigned short     FOE_Ack(unsigned int fileOffset, unsigned short  * pData);
    unsigned short     FOE_Busy(unsigned short done, unsigned int fileOffset, unsigned short  * pData);
    void       FOE_Error(unsigned int errorCode);


 
#line 56 "etherCAT\\foeappl.c"
...
unsigned short FOE_Data(unsigned short  * pData, unsigned short Size)
{
    unsigned char bDataFollowing = 0;

    if(Size == (u16ReceiveMbxSize - 6 - sizeof(TFOEHEADER)) )
    {
        bDataFollowing = 1;
    }

    if(pAPPL_FoeWriteData != 0)
    {
        ...
        else if (bDataFollowing == 0)
        {
            return ((((((((0x8000)-1))-1))-1))-1);
        }
