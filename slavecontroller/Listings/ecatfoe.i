...
#line 135 "etherCAT\\ecatfoe.h"
    ...
    unsigned int                                 u32FileOffset;   
    unsigned int                                 u32LastFileOffset;  
    unsigned short                                 u16FileAccessState;  
    TMBX  *                          pFoeSendStored; 
   ...
   unsigned short                                  u16FoeMaxSendBlockSize;   
    ...
    void     FOE_Init(void);
    unsigned char FOE_ServiceInd(TFOEMBX  * pFoeInd);
    void     FOE_ContinueInd(TMBX  * pMbx);


 
#line 65 "etherCAT\\ecatfoe.c"
...
unsigned char FOE_ServiceInd(TFOEMBX  * pFoeInd)
{
    ...
    unsigned short nextState = 0x8004;
     
    unsigned short dataSize = (pFoeInd->MbxHeader . Length) - sizeof(TFOEHEADER);


    
 
    if ( (pFoeInd->MbxHeader . Length) < sizeof(TFOEHEADER) )
    {
        return 0x06;
    }

    switch ( (pFoeInd->FoeHeader . OpCode) )
    {
    case 1:
         
        if ( u16FileAccessState == 0 )
        {
            unsigned int u32Password = ((((unsigned int)pFoeInd->FoeHeader . Cmd . Password[0]) | ((unsigned int)pFoeInd->FoeHeader . Cmd . Password[1]) << 16));
             
            nextState = FOE_Read((unsigned short  *)pFoeInd->Data, dataSize, (unsigned short*)pFoeInd->Data, u32Password);

             
            u32LastFileOffset = 0;

            if (nextState <= ((((((((((((((((0x8000)-1))-1))-1))-1))-1))-1))-100))-1))
            {
            ...
            else if (nextState <= ((((((((((((0x8000)-1))-1))-1))-1))-1))-1))
            {
                u32PacketNo    = 0;
                u32FileOffset = 0;
                u16FileAccessState = 1;
            }
        }
        break;
    ...
    case 2:
        ...
        if ( u16FileAccessState == 0 )
        {
            ...
            nextState = FOE_Write((unsigned short  *)pFoeInd->Data, dataSize, u32Password);
            if ( nextState == 0 )
            {
                 
                nextState = ((((((0x8000)-1))-1))-1);
            }
    ...
    case 3:
        ...
        if ( u16FileAccessState == 2
          || u16FileAccessState == 6 )
        {
            unsigned int u32CmdPacketNo = ((((unsigned int)pFoeInd->FoeHeader . Cmd . PacketNo[0]) | ((unsigned int)pFoeInd->FoeHeader . Cmd . PacketNo[1]) << 16));
            ...
            if ( u32CmdPacketNo == u32PacketNo )
            {
                ...
                nextState = FOE_Data((unsigned short  *)pFoeInd->Data, dataSize);
                if ( nextState == 0 )
                {
    ...
    case 4:
        ...
        if ( u16FileAccessState == 1 )
        {
            
 
            nextState = FOE_Ack( u32FileOffset, (unsigned short  *)pFoeInd->Data );
        ...
        else if ( u16FileAccessState == 4 )
        {
             
            nextState = ((((0x8000)-1))-1);
        }
        break;
    ...
    case 5:
        ...
        if ( u16FileAccessState != 0 )
        {
            unsigned int u32CmdErrorCode = ((((unsigned int)pFoeInd->FoeHeader . Cmd . ErrorCode[0]) | ((unsigned int)pFoeInd->FoeHeader . Cmd . ErrorCode[1]) << 16));
    ...
    case 6:
        ...
        if ( u16FileAccessState == 1
          || u16FileAccessState == 4 )
        {
            
 
            nextState = FOE_Busy( (pFoeInd->FoeHeader . Cmd . Busy . Done), u32LastFileOffset, (unsigned short  *)pFoeInd->Data );
        }
        break;
    ...
    if ( nextState <= ((((((((((((((((0x8000)-1))-1))-1))-1))-1))-1))-100))-1) )
    {
        ...
        unsigned int d = (u32PacketNo);

         
        pFoeInd->FoeHeader.OpCode           = (3);
         
        pFoeInd->FoeHeader.Cmd.PacketNo[0] = ((d & 0x0000FFFF));
        pFoeInd->FoeHeader.Cmd.PacketNo[1] = ((d & 0xFFFF0000) >> 16);
     
         
        pFoeInd->MbxHeader.Length           = sizeof(TFOEHEADER) + nextState;

        if ( nextState == u16SendMbxSize - sizeof(TFOEHEADER) - 6 )
        {
    ...
    else if ( nextState <= ((((((((((((0x8000)-1))-1))-1))-1))-1))-1) )
    {
        ...
        pFoeInd->FoeHeader.OpCode                   = (6);
         
        pFoeInd->FoeHeader.Cmd.Busy.Done            = (nextState -((((((((((((((0x8000)-1))-1))-1))-1))-1))-1))-100));
    ...
    else if ( nextState == ((((((0x8000)-1))-1))-1) || nextState == ((((((((0x8000)-1))-1))-1))-1) )
    {
        ...
        if ( nextState == ((((((0x8000)-1))-1))-1) )
        {
             
            u16FileAccessState = 2;
        }
        else
        {
    ...
    {
        unsigned int d = (nextState);
        unsigned char b = 0;

         
        pFoeInd->FoeHeader.OpCode                    = (5);
         
        pFoeInd->FoeHeader.Cmd.ErrorCode[0] = ((d & 0x0000FFFF));
        pFoeInd->FoeHeader.Cmd.ErrorCode[1] = ((d & 0xFFFF0000) >> 16);
        ...
        while (b < 32)
        {
            unsigned short data = ((unsigned short  *) pFoeInd->Data)[(b >> 1)];
    ...
    if ( bBootMode )
    {
        ...
        pFoeInd->MbxHeader.Flags[0] &= ~0xF000;
        ...
        pFoeInd->MbxHeader.Flags[0] |= u8MbxReadCounter << 12;
         
          u8MbxReadCounter++;
        

 
        MBX_CopyToSendMailbox((TMBX  *) pFoeInd);
    }
    else
    {
        if ( MBX_MailboxSendReq((TMBX  *) pFoeInd, ((unsigned char) 0x0040)) != 0 )
        {
            


 
            pFoeSendStored = (TMBX  *) pFoeInd;
        }
    }
...
void FOE_ContinueInd(TMBX  * pMbx)
{
    if ( pFoeSendStored )
    {
        ...
        if (MBX_MailboxSendReq(pFoeSendStored, ((unsigned char) 0x0040)) == 0)
        {
            pFoeSendStored = 0;
        }
    }
