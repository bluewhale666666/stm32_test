...
#line 350 "etherCAT\\ecatslv.h"
 ...
 unsigned char                             u8LocalErrorState;  
 ...
 unsigned char                             u8EcatErrorLed;  

 unsigned char                             u8EcatRunLed;  
 ...
 unsigned char                              bExplicitDevIdRequested;  
...
#line 208 "etherCAT\\ecatslv.c"


#line 1 "etherCAT\\bootmode.h"
...
extern    void BL_StartDownload(unsigned int password);
extern   unsigned short BL_Data(unsigned short *pData,unsigned short Size);


 
#line 211 "etherCAT\\ecatslv.c"
...
#line 219 "etherCAT\\ecatslv.c"

#line 1 "etherCAT\\emcy.h"
...
typedef struct
{
    unsigned short                      FirstInQueue;  
    unsigned short                      LastInQueue;  
    unsigned short                      PutInQueueFailedCounter;
    unsigned short                      MaxQueueSize;  
    TEMCYMESSAGE  *      pQueue[0x1+1];  
} TEMCYQUEUE;
...
extern    TEMCYQUEUE      sSendEmcyQueue;  
extern    TEMCYQUEUE      sEmptyEmcyQueue;  
extern    TEMCYMESSAGE    asEmcy[(0x1)];  
...
extern    void PutInEmptyEmcyQueue(TEMCYMESSAGE  * pEmcy);
extern    TEMCYMESSAGE  * GetOutOfEmptyEmcyQueue(void);
extern    void PutInSendEmcyQueue(TEMCYMESSAGE  * pEmcy);
extern    TEMCYMESSAGE  * GetOutOfSendEmcyQueue(void);
extern    void EMCY_Init(void);
extern    unsigned char EMCY_IsQueueEmpty(void);
extern    void EMCY_ContinueInd(TMBX  * pMbx);
extern    TEMCYMESSAGE  * EMCY_GetEmcyBuffer(void);
extern    unsigned char EMCY_SendEmergency(TEMCYMESSAGE  *pEmcy);


 
#line 221 "etherCAT\\ecatslv.c"
...
extern TOBJ1600 OUT_GENERICProcessDataMapping0x1600
...
extern TOBJ1A00 IN_GENERICProcessDataMapping0x1A00
...
extern TOBJ6000 IN_GENERIC0x6000
...
extern TOBJ7000 OUT_GENERIC0x7000
...
extern TOBJ8000 ParameterConfiguration0x8000
...
extern unsigned short APPL_GetDeviceID(void);
...
unsigned short	  u16IdValue;						    
 

 
 
    volatile unsigned short    u16dummy;
 

 

 
        volatile unsigned short SMActivate = 0;
...
void ResetALEventMask(unsigned short intMask)
{
    ...
    unsigned short mask;
    HW_EscRead(((unsigned char *)&(mask)),((unsigned short)(0x0204)),2);
    ...
    HW_EscWrite(((unsigned char *)&(mask)),((unsigned short)(0x0204)),2);
...
void UpdateEEPROMLoadedState(void)
{
    ...
    HW_EscRead(((unsigned char *)&(TmpVar)),((unsigned short)(0x0502)),2);
    TmpVar = (TmpVar);
...
TSYNCMAN  * GetSyncMan( unsigned char channel )
{
    HW_EscRead((unsigned char *)&SyncManInfo, 0x0800 + (channel * 8), 8 );
...
void DisableSyncManChannel(unsigned char channel)
{
    ...
    volatile unsigned short smStatus = 0x0100;
    Offset = (0x0806 + (8*channel));

 

    HW_EscWrite(((unsigned char *)&(smStatus)),((unsigned short)(Offset)),2);
    
     
    do
    {
        HW_EscRead(((unsigned char *)&(smStatus)),((unsigned short)(Offset)),2);
 
    }while(!(smStatus & 0x0100));
...
void EnableSyncManChannel(unsigned char channel)
{
    ...
    volatile unsigned short smStatus = 0x0000;
    Offset = (0x0806 + (8*channel));
...
void    SendSmFailedEmergency(unsigned char channel, unsigned char faultyCode)
{
    TEMCYMESSAGE  *    pEmcy    = EMCY_GetEmcyBuffer();
    ...
    if ( pEmcy )
    {
        ...
        pEmcy->Emcy.RegData[0] &= ~0xFF00;
        pEmcy->Emcy.RegData[0] |= ((faultyCode)+((channel)<<2)) << 8;
        ...
        case 2:
            ...
            case 0x01:
                ...
                pEmcy->Emcy.RegData[1] = (0x1000);
                 
                pEmcy->Emcy.RegData[1+1] = (0x1FFF);
                break;
            case 0x00:
                 
                pEmcy->Emcy.RegData[1] = (nPdOutputSize);
                 
                pEmcy->Emcy.RegData[1+1] = (nPdOutputSize);
            ...
            case 0x02:
                ...
                if (nPdOutputSize)
                {
                    pEmcy->Emcy.RegData[1] = (0xA); 
 
                    pEmcy->Emcy.RegData[1+1] = (0x0001);
                }
                else
                {
                    pEmcy->Emcy.RegData[1] = 0;
                    pEmcy->Emcy.RegData[1+1] = 0;
                }
                break;
            }
            break;
        case 3:
            ...
            case 0x00:
                ...
                pEmcy->Emcy.RegData[1]   = (nPdInputSize);
                 
                pEmcy->Emcy.RegData[1+1] = (nPdInputSize);
        ...
        pEmcy->Emcy.Code = (0xA000);
         

        pEmcy->Emcy.RegData[0] &= ~0x00FF;
        pEmcy->Emcy.RegData[0] |= nAlStatus << 0;
...
unsigned char    CheckSmSettings(unsigned char maxChannel)
{
    ...
    unsigned short SMAddress = 0;
        ...
        if ((nMaxEscAddress < 0x1FFF)
            || (nMaxEscAddress < 0x1FFF)
            || (nMaxEscAddress < 0x1FFF)
            || (nMaxEscAddress < 0x1FFF))
        {
    ...
    if (!(pSyncMan->Settings[1] & 0x0001))
    {
         
        result = 0x0016;
    }
    else if ((pSyncMan->Settings[0] & 0x000C) != 0x0004)
    {
         
        result = 0x0016;
    }
    else if ((pSyncMan->Settings[0] & 0x0002) != 0x0002)
    {
    ...
    else if ((pSyncMan->Settings[0] & 0x000C) != 0x0000)
    {
    ...
    if ((pSyncMan->Settings[1] & 0x0001) != 0 && SMLength == 0)
    {
         
        result = 0x02 + 1;
    }
        else if (pSyncMan->Settings[1] & 0x0001)
        {
             
                if (SMLength != nPdInputSize || nPdInputSize == 0 || SMLength > 0x0100)
                {
                ...
                else
                {
                    ...
                    if ((pSyncMan->Settings[0] & 0x000C) == 0x0000)
                    {
                         
                        if (((nAlStatus == ((unsigned char) 0x02)) && (SMAddress >= 0x1000) && (SMAddress <= 0x1FFF))
                            || ((nAlStatus != ((unsigned char) 0x02)) && (SMAddress == nEscAddrInputData))
                            )
                        {
                             

                                if ((pSyncMan->Settings[0] & 0x0002) == 0x0002)
                                {
        ...
        if ( result != 0 )
        {
            ...
            SendSmFailedEmergency(3, result);
            result = 0x001E;
        }
    }
    ...
    if ((pSyncMan->Settings[1] & 0x0001) != 0 && SMLength == 0)
    {
        ...
        else if (pSyncMan->Settings[1] & 0x0001)
        {
            ...
            if ( SMLength == nPdOutputSize && nPdOutputSize != 0 && SMLength <= ((unsigned short)0x0100))
            {
                 
                if ( (pSyncMan->Settings[0] & 0x000C) == 0x0004 )
                {
        ...
        if ( result != 0 )
        {
            ...
            SendSmFailedEmergency(2, result);
            result = 0x001D;
        }
    }
    ...
    if ( result == 0 )
    {
        ...
        for (i = maxChannel; i < nMaxSyncMan; i++)
        {
            ...
            SMActivate = pSyncMan->Settings[1];
        }
    }
    return result;
}
...
unsigned short StartInputHandler(void)
{
    ...
    unsigned short        dcControl;
 

    unsigned short     wdiv = 0;
    unsigned short     wd = 0;
    unsigned int     cycleTimeSync0 = 0;  
    unsigned int     shiftTimeSync1 = 0;  
        ...
        )
    {
        ...
        SendSmFailedEmergency(3, 0x01+1);
        return 0x001E;
    }
        ...
        )
    {
        ...
        SendSmFailedEmergency(2, 0x01+1);
        return 0x001D;
    }
    ...
    HW_EscRead(((unsigned char *)&(dcControl)),((unsigned short)(0x0980)),2);
    dcControl = (dcControl);
 

    
        HW_EscRead(((unsigned char *)&(cycleTimeSync0)),((unsigned short)(0x09A0)),4);
        cycleTimeSync0 = (cycleTimeSync0);

    
        HW_EscRead(((unsigned char *)&(shiftTimeSync1)),((unsigned short)(0x09A4)),4);
    ...
    if((dcControl & (0x0100 | 0x0800)) != 0)
    {
         
        if((dcControl & (0x0200 | 0x0400)) == 0)
        {
        ...
        {
            if((((u16MinSuppSyncType & 0x0004) == 0) && ((dcControl & 0x0200) != 0))
                ||(((u16MinSuppSyncType & 0x0008) == 0) && ((dcControl & 0x0400) != 0)))
            {
        ...
        {
            unsigned int curMinCycleTime = 0;
        ...
        if(((dcControl & 0x0200) != 0) && ((dcControl & 0x0400) != 0))
        {
    ...
    {
        sSyncManOutPar.u32Sync0CycleTime = (unsigned int)cycleTimeSync0;
        sSyncManOutPar.u32CycleTime = (unsigned int)cycleTimeSync0;

        sSyncManInPar.u32Sync0CycleTime = (unsigned int)cycleTimeSync0;
        sSyncManInPar.u32CycleTime = (unsigned int)cycleTimeSync0;
    }
    ...
    if(bSubordinatedCycles == 1)
    {
        ...
        unsigned int cycleTimeSync1 = (shiftTimeSync1 + cycleTimeSync0);
    ...
    HW_EscRead(((unsigned char *)&(wd)),((unsigned short)(0x0420)),2);
    wd = (wd);
    ...
    HW_EscRead(((unsigned char *)&(wdiv)),((unsigned short)(0x0400)),2);
    wdiv = (wdiv);
        if ( wdiv != 0 )
        {
             
            unsigned int d = wdiv+2;
    ...
    {
        unsigned int u32TmpVar = 0;
        HW_EscRead(((unsigned char *)&(u32TmpVar)),((unsigned short)(0x0150)),4);
        if ((u32TmpVar & 0x0800) > 0)
        {
            u16ALEventMask |= ((unsigned short) 0x04);
        }

        if ((u32TmpVar & 0x8000) > 0)
        {
            u16ALEventMask |= ((unsigned short) 0x08);
        }
    ...
    if ( (dcControl & 0x0200) != 0 )
    {
        ...
        {
            unsigned int Sync0Cycle = cycleTimeSync0/100000;
        ...
        else
        {
                ...
                unsigned int Sync1Cycle = (shiftTimeSync1  + cycleTimeSync0 )/100000;
                if(Sync1Cycle < 5)
                {
...
unsigned short StartOutputHandler(void)
{
    ...
    if(((((u8LocalErrorState) & ((unsigned char) 0x0F)) == ((unsigned char) 0x01)) || (((u8LocalErrorState) & ((unsigned char) 0x0F)) == ((unsigned char) 0x02)) || (((u8LocalErrorState) & ((unsigned char) 0x0F)) == ((unsigned char) 0x04)) || (((u8LocalErrorState) & ((unsigned char) 0x0F)) == ((unsigned char) 0x08))))
...
void SetALStatus(unsigned char alStatus, unsigned short alStatusCode)
{
    ...
    if(bExplicitDevIdRequested && !(nAlStatus & ((unsigned char) 0x10)) && alStatusCode == 0 && ((nAlStatus & ((unsigned char) 0x0F)) != ((unsigned char) 0x03)))
    {
 
        Value = u16IdValue;
 
        nAlStatus |= ((unsigned char) 0x20);
    }
    else
    {
        nAlStatus &= ~((unsigned char) 0x20);
    ...
    if (alStatusCode != 0xFFFF)
    {
        ...
        HW_EscWrite(((unsigned char *)&(Value)),((unsigned short)(0x0134)),2);
    }

 
    Value = nAlStatus;
    Value = (Value);
    HW_EscWrite(((unsigned char *)&(Value)),((unsigned short)(0x0130)),2);

     

     
    if(alStatusCode == 0x00 || !(alStatus & ((unsigned char) 0x10)))
    {
        u8EcatErrorLed = 0x00;
    }
    else if((alStatusCode == 0x002D) ||
        (alStatusCode == 0x001A) ||
        (alStatusCode == 0x0032)
    ...
    switch((alStatus & ((unsigned char) 0x0F)))
    {
    ...
    {
    unsigned short TmpVar = 0;
    TmpVar = ((((unsigned short)u8EcatRunLed) | (((unsigned short)u8EcatErrorLed)<<8)));
    HW_EscWrite(((unsigned char *)&(TmpVar)),((unsigned short)(0x0138)),2);
    }
        ...
        if(EsmTimeoutCounter == 0)
        {
            ...
            switch(nEcatStateTrans)
            {
                ...
                case ((unsigned char)((((unsigned char) 0x01)) << 4) | (((unsigned char) 0x03))):
                    ...
                    if((u8LocalErrorState & ((unsigned char) 0x0F)) == ((unsigned char) 0x01))
                ...
                case ((unsigned char)((((unsigned char) 0x02)) << 4) | (((unsigned char) 0x04))):
                    ...
                    if ((u8LocalErrorState & ((unsigned char) 0x0F)) == ((unsigned char) 0x02))
                ...
                case ((unsigned char)((((unsigned char) 0x04)) << 4) |( ((unsigned char) 0x08))):
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if ((u8LocalErrorState & ((unsigned char) 0x0F)) == ((unsigned char) 0x04))
                            {
...
void CheckIfEcatError(void)
{
   ...
   if (EcatWdValue != 0)
   {
      ...
      HW_EscRead(((unsigned char *)&(WdStatusOK)),((unsigned short)(0x0440)),2);
...
void ECAT_StateChange(unsigned char alStatus, unsigned short alStatusCode)
{
    ...
    if (alStatusCode != 0 && !((((alStatus) & ((unsigned char) 0x0F)) == ((unsigned char) 0x01)) || (((alStatus) & ((unsigned char) 0x0F)) == ((unsigned char) 0x02)) || (((alStatus) & ((unsigned char) 0x0F)) == ((unsigned char) 0x04)) || (((alStatus) & ((unsigned char) 0x0F)) == ((unsigned char) 0x08))))
    {
        return;
    }

     
    if (bEcatWaitForAlControlRes == 0 && u8LocalErrorState == alStatus && u16LocalErrorCode == alStatusCode)
    {
        return;
    }
    ...
    if(bEcatWaitForAlControlRes)
    {
        ...
        if(bApplEsmPending)
        {
            ...
            if(alStatusCode != 0)
            {
                ...
                u8LocalErrorState = (alStatus & ((unsigned char) 0x0F));
    ...
    else
    {
        ...
        if ( alStatusCode != 0 && ((alStatus & ((unsigned char) 0x0F)) != ((unsigned char) 0x08)) && ((((alStatus) & ((unsigned char) 0x0F)) == ((unsigned char) 0x01)) || (((alStatus) & ((unsigned char) 0x0F)) == ((unsigned char) 0x02)) || (((alStatus) & ((unsigned char) 0x0F)) == ((unsigned char) 0x04)) || (((alStatus) & ((unsigned char) 0x0F)) == ((unsigned char) 0x08))))
        {
            u8LocalErrorState = (alStatus & ((unsigned char) 0x0F));
            ...
            if ((nAlStatus & ((unsigned char) 0x0F)) == ((unsigned char) 0x08))
            {
...
void ECAT_Init(void)
{
    ...
    HW_EscRead(((unsigned char *)&(TmpVar)),((unsigned short)(0x0004)),2);

    TmpVar = (TmpVar);
    nMaxSyncMan = (unsigned char) ((TmpVar & 0xFF00)>> 8);

    HW_EscRead(((unsigned char *)&(TmpVar)),((unsigned short)(0x0006)),2);
    TmpVar = (TmpVar);

    
    nMaxEscAddress = (unsigned short) ((TmpVar & 0x00FF) << 10) + 0xFFF;
    }
...
void ECAT_Main(void)
{
    ...
    unsigned short sm1Activate = 0x0001;
    unsigned short sm1Status = 0;  
    ...
    if ( bMbxRunning )
    {
        ...
        HW_EscRead(((unsigned char *)&(sm1Activate)),((unsigned short)((0x0806 + 8))),2);
        sm1Activate = (sm1Activate);

        HW_EscRead(((unsigned char *)&(sm1Status)),((unsigned short)((0x0804 + 8))),2);
        sm1Status = (sm1Status);
    ...
    if ((ALEventReg & ((unsigned short) 0x01)) && !bEcatWaitForAlControlRes)
    {
        ...
        HW_EscRead(((unsigned char *)&(EscAlControl)),((unsigned short)(0x0120)),2);
        EscAlControl = (EscAlControl);
            ...
            if ((EscAlControl & (unsigned short)((unsigned char) 0x20)) == ((unsigned char) 0x20))
            {
                if (bExplicitDevIdRequested == 0)
                {
                    u16IdValue = APPL_GetDeviceID();
                }

                bExplicitDevIdRequested = 1;
            }
            else
            {
                bExplicitDevIdRequested = 0;
    ...
    if ( bMbxRunning )
    {
            ...
            if (!(sm1Activate & 0x0001))
            {
                AL_ControlInd(nAlStatus & ((unsigned char) 0x0F), 0);
            }

 
        if (((sm1Status & 0x0800) == 0)
            && bSendMbxIsFull) 
 
        {
            ...
            u16dummy = 0;
            HW_EscWrite(((unsigned char *)&(u16dummy)),((unsigned short)(u16EscAddrSendMbx)),2);
            ...
            if (((sm1Activate & 0x0002) && !bMbxRepeatToggle)
                || (!(sm1Activate & 0x0002) && bMbxRepeatToggle))
            {
                ...
                sm1Activate = (sm1Activate);
                HW_EscWrite(((unsigned char *)&(sm1Activate)),((unsigned short)((0x0806 + 8))),2);
