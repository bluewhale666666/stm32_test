...
static uint16_t crc_table[256] = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5,
...
HAL_StatusTypeDef Power_Command_Transmit(uint8_t ReqCode)
{
	...
	for(i=0u; i<(uint32_t)6u;i++){
		if(ReqCode == strPwrCmdTxReqMap[i].SM2SU_REQ_CD){
			...
			ans = HAL_UART_Transmit(Uart_HandlePtr, PowerCommandTxBuffer, PwrCmdTxSize, (uint32_t)500u);
			break;
		}
	}
	return(ans);
...
static void SM2SU_Req_Call_Encapsulation_Func(uint8_t ReqCd, uint8_t *pData, uint16_t *pSize, uint16_t InfoLength)
{
		...
		*(pSize) = ((4u) + (4u) + 																	(1u) + (1u) + 																	InfoLength + (2u));
		...
		pData[(4u) + 					(1u) + (1u) + 					InfoLength] = (uint8_t)(crc16 & 0xff); 	


		pData[(4u) + 					(1u) + (1u) + 					InfoLength + 1u] = (uint8_t)((crc16 >> 8)& 0xff);


	
		
		pData[(4u) + 					(1u) + (1u) + 					InfoLength + (2u)] = 0xEEu;


		pData[(4u) + 					(1u) + (1u) + 					InfoLength + (2u) + 1u] = 0x55u;


		pData[(4u) + 					(1u) + (1u) + 					InfoLength + (2u) + 2u] = 0x55u;


		pData[(4u) + 					(1u) + (1u) + 					InfoLength + (2u) + 3u] = 0xEEu;
