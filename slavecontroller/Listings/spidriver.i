...
#line 47 "etherCAT\\SPIDriver.c"

SPI_HandleTypeDef *EcatHspiPtr;
unsigned char EcatSpiReadBuffer[512u];
unsigned char EcatSpiWriteBuffer[512u];

static void SPIWriteBytes(unsigned short Address, unsigned char *Val, unsigned char nLenght);
void CSHIGH(void);
void CSLOW(void);
...
unsigned int SPIReadDWord (unsigned short Address)
{
    ...
    EcatSpiWriteBuffer[0u] = (uint8_t)0x0B;
    EcatSpiWriteBuffer[1u] = wAddr.byte.HB;
    EcatSpiWriteBuffer[2u] = wAddr.byte.LB;
    EcatSpiWriteBuffer[3u] = (uint8_t)1;


		CSLOW();
    HAL_SPI_TransmitReceive(EcatHspiPtr, EcatSpiWriteBuffer, EcatSpiReadBuffer, 8u, 2000u);
		CSHIGH();



	  dwResult.Val = *((unsigned int *)(&EcatSpiReadBuffer[4u]));
...
static void SPIWriteBytes(unsigned short Address, unsigned char *Val, unsigned char nLenght)
{
    ...
    else
    {
        ...
        EcatSpiWriteBuffer[0u] = (uint8_t)0x02;
        EcatSpiWriteBuffer[1u] = (wAddr.byte.HB|(uint8_t)0x40);
        EcatSpiWriteBuffer[2u] = wAddr.byte.LB;
        memcpy(&EcatSpiWriteBuffer[3u], dwData, nLenght);
				

				CSLOW();
				HAL_SPI_TransmitReceive(EcatHspiPtr, EcatSpiWriteBuffer, EcatSpiReadBuffer, size, 2000u);
				CSHIGH();
...
void SPIReadRegUsingCSR(unsigned char *ReadBuffer, unsigned short Address, unsigned char Count)
{
    ...
    param32_1.Val = SPIReadDWord (0x300);
...
void SPIWriteRegUsingCSR( unsigned char *WriteBuffer, unsigned short Address, unsigned char Count)
{
    ...
    SPIWriteDWord (0x300, param32_1.Val);
...
void SPIReadPDRamRegister(unsigned char *ReadBuffer, unsigned short Address, unsigned short Count)
{
    ...
    unsigned char i = 0,nlength, nBytePosition;
    unsigned char nReadSpaceAvblCount;
    unsigned short RefAddr = Address;
    UINT16_VAL wAddr;
    unsigned short j;
		unsigned short size;
    ...
    SPIWriteDWord (0x0300+0x0c, param32_1.Val);

    
 
	do
    {
        param32_1.Val = SPIReadDWord (0x0300+0x0c);

    }while((param32_1.v[3] & (1 << 7)));
	...
	SPIWriteBytes (0x0300+0x08, (unsigned char*)&param32_1.Val,8);   
    ...
    nReadSpaceAvblCount = param32_1.v[1] & 0x1f;
    ...
    param32_1.Val = SPIReadDWord (0x04);
    nReadSpaceAvblCount--;
...
{
    UINT64_VAL param32_1 = {0};
    unsigned char i = 0,nlength, nBytePosition,nWrtSpcAvlCount;
    UINT16_VAL wAddr;
    unsigned short j;
    unsigned short size;
    ...
    SPIWriteDWord (0x0300+0x14, param32_1.Val);

    
 
    do
    {
        param32_1.Val = SPIReadDWord (0x0300+0x14);
   ...
   SPIWriteBytes (0x0300+0x10, (unsigned char*)&param32_1.Val,8);
    ...
    nWrtSpcAvlCount = param32_1.v[1] & 0x1f;
...
void CSHIGH(void)
{
	 ...
	 ((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x1800UL))->ODR |= ((uint32_t)1u << 4u);
...
void CSLOW(void)
{
	 ...
	 ((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x1800UL))->ODR &= 0xffefu;
