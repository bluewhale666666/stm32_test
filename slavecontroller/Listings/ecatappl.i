...
#line 85 "etherCAT\\ecatappl.h"
 ...
 unsigned char bEcatWaitForInputUpdate;  
 unsigned char bEtherCATRunLed;  
 unsigned char bEtherCATErrorLed;  
 unsigned char bRunApplication;  
    ...
    unsigned short     ESC_EepromAccess(unsigned int wordaddress, unsigned short wordsize, unsigned short  *pData, unsigned char access);
    unsigned short     ESC_EepromWriteCRC(void);
...
extern unsigned int u32LastDc32Value;
extern unsigned int u32CheckForDcOverrunCnt;
...
#line 54 "etherCAT\\applInterface.h"
 ...
 unsigned short (*pAPPL_FoeRead)(unsigned short  * pName, unsigned short nameSize, unsigned int password, unsigned short maxBlockSize, unsigned short *pData);
 ...
 unsigned short(*pAPPL_FoeReadData)(unsigned int offset, unsigned short maxBlockSize, unsigned short *pData);
 ...
 void(*pAPPL_FoeError)(unsigned int errorCode);
 ...
 unsigned short (*pAPPL_FoeWrite)(unsigned short  * pName, unsigned short nameSize, unsigned int password);
 ...
 unsigned short(*pAPPL_FoeWriteData)(unsigned short  * pData, unsigned short Size, unsigned char bDataFollowing);
 ...
 void(*pAPPL_CoeReadInd)(unsigned short Index, unsigned char Subindex, unsigned char CompleteAccess);
 ...
 void(*pAPPL_CoeWriteInd)(unsigned short Index, unsigned char Subindex, unsigned char CompleteAccess);
...
unsigned int u32CycleTimeStartValue;  

unsigned int u32MinCycleTimeStartValue;  

unsigned int u32SystemTimeReadFailure;  

unsigned char bMinCycleTimeMeasurementStarted;  

unsigned int u32MinCycleTimeValue;  
...
unsigned short             aPdOutputData[(0x0100>>1)];
unsigned short           aPdInputData[(0x0100>>1)];

 
 
    extern volatile unsigned short    u16dummy;
 

unsigned char bInitFinished = 0;  
...
unsigned int GetSystemTimeDelay(unsigned int u32StartTime);
...
void PDO_InputMapping(void)
{
    ...
    unsigned int u32TimeValue = 0;
    unsigned short ALEvent = HW_GetALEventRegister_Isr();
    ALEvent = (ALEvent);

    if ((((sSyncManOutPar . u16GetCycleTime & 0x1) == 0x1) || ((sSyncManInPar . u16GetCycleTime & 0x1) == 0x1)))
    {
    ...
    APPL_InputMapping((unsigned short*)aPdInputData);
    HW_EscWriteIsr(((unsigned char *) aPdInputData), nEscAddrInputData, nPdInputSize );
    ...
    if ((((sSyncManOutPar . u16GetCycleTime & 0x1) == 0x1) || ((sSyncManInPar . u16GetCycleTime & 0x1) == 0x1)))
    {
        ...
        if (bMinCycleTimeMeasurementStarted == 1)
        {
            ...
            bMinCycleTimeMeasurementStarted = 0;
        }
...
void PDO_OutputMapping(void)
{
   ...
   if ((((sSyncManOutPar . u16GetCycleTime & 0x1) == 0x1) || ((sSyncManInPar . u16GetCycleTime & 0x1) == 0x1)))
   {
      ...
      bMinCycleTimeMeasurementStarted = 1;
    ...
    HW_EscReadIsr(((unsigned char *)aPdOutputData), nEscAddrOutputData, nPdOutputSize );
    APPL_OutputMapping((unsigned short*) aPdOutputData);
...
unsigned int GetSystemTimeDelay(unsigned int u32StartTime)
{
   unsigned int u32CurValue = 0;
   unsigned int u32Delta = 0;

   
   HW_EscReadIsr(((unsigned char *)&(u32CurValue)),((unsigned short)(0x0910)),4);
...
void PDI_Isr(void)
{
    ...
    unsigned char SyncAcknowledgePending = 0;
    ...
    if (ALEvent & ((unsigned short) 0x08))
    {
        Sync1_Isr();

        SyncAcknowledgePending = 1;
    }
    ...
    if(bEscIntEnabled)
    {
        if ( ALEvent & ((unsigned short) 0x0400) )
        {
        ...
        else
        {
            ...
            HW_EscReadIsr(((unsigned char *)&(u16dummy)),((unsigned short)(nEscAddrOutputData)),2);
            HW_EscReadIsr(((unsigned char *)&(u16dummy)),((unsigned short)((nEscAddrOutputData+nPdOutputSize - 2))),2);
        ...
        if (sSyncManOutPar.u16SyncType == 0x0001)
        {
    ...
    if ( (bEcatInputUpdateRunning == 1) && (nPdInputSize > 0)
 
       && ((sSyncManInPar.u16SyncType == 0x0001) || (sSyncManInPar.u16SyncType == 0x0022))
    ...
    if (ALEvent & ((unsigned short) 0x04))
    {
        Sync0_Isr();
    ...
    {
        volatile unsigned int SyncState = 0;
        HW_EscRead(((unsigned char *)&(SyncState)),((unsigned short)(0x098C)),4);
...
void Sync0_Isr(void)
{
    ...
    if(bDcSyncActive)
    {
        ...
        unsigned char bCallInputMapping = 0;
 
 

 
        if ((bEcatInputUpdateRunning == 1) && (LatchInputSync0Value > 0) && (nPdInputSize > 0))
        {
        ...
        if(u16SmSync0Value > 0)
        {
           ...
           if ((nPdOutputSize == 0) && (nPdInputSize > 0))
           {
              ...
              if ((ALEvent & ((unsigned short) 0x0800)) == 0)
              {
...
void Sync1_Isr(void)
{
    ...
    if ( (bEcatInputUpdateRunning == 1) && (nPdInputSize > 0)
            ...
            && (sSyncManInPar.u16SyncType == 0x0003)
...
unsigned short MainInit(void)
{
    ...
    unsigned short eepromConfigControl = 0; 

    HW_EscRead(((unsigned char *)&(eepromConfigControl)),((unsigned short)(0x0500)),2);
    eepromConfigControl = (eepromConfigControl);

    if((eepromConfigControl & 0x0001) > 0)
    {
         
        eepromConfigControl &= ~0x0100;

        eepromConfigControl = (eepromConfigControl);
        HW_EscWrite(((unsigned char *)&(eepromConfigControl)),((unsigned short)(0x0500)),2);
    }
    }
     ...
     
    {
       ...
       while (u32Cnt < 1000)
       {
           HW_EscReadIsr(((unsigned char *)&(u32TimeValue)),((unsigned short)(0x0910)),4);
           HW_EscReadIsr(((unsigned char *)&(u32Delta)),((unsigned short)(0x0910)),4);
...
void MainLoop(void)
{
    ...
    if(bInitFinished == 0)
    {
        ...
        if (
            (!bEscIntEnabled || !bEcatFirstOutputsReceived)      
            ...
            )
        {
            ...
            if ( !bEscIntEnabled )
            {
                ...
                unsigned short ALEvent = HW_GetALEventRegister();
                ALEvent = (ALEvent);
                ...
                else if ( nPdOutputSize == 0 )
                {
                    ...
                    if ( ALEvent & ((unsigned short) 0x0800) )
                    {
        ...
        {
            unsigned int CurTimer = (unsigned int)(PDI_GetTimer());
        ...
        if (u32CheckForDcOverrunCnt >= 4000)
        {
            COE_SyncTimeStamp();
        }
...
{
    unsigned short i;
    unsigned char u8TimeOut;
    unsigned short u16RetErr = 0;
    unsigned short u16WordOffset = 0;
    unsigned char RetryCnt = 10; 

 
    unsigned short nConfig;
    volatile unsigned short nControl;

    HW_EscRead(((unsigned char *)&(nConfig)),((unsigned short)(0x0500)),2);
    HW_EscRead(((unsigned char *)&(nControl)),((unsigned short)(0x0502)),2);
    ...
    if ( nConfig & 0x0001 )
    {
        ...
        HW_EscWrite(((unsigned char *)&(nConfig)),((unsigned short)(0x0500)),2);
        for (i = 0; i < wordsize;)
        {
             
 
 
            HW_EscWrite((unsigned char *) &wordaddress, 0x0504, 4);
            ...
            if ( access == 0 )
            {
                ...
                HW_EscWrite(((unsigned char *)&(nControl)),((unsigned short)(0x0502)),2);
            }
            else
            {
                 
 
                HW_EscWrite(((unsigned char *)&(pData[i])),((unsigned short)(0x0508)),2);
            ...
            else
            {
                if ( access == 0 )
                {
                    unsigned short u16BytesToCopy = (step << 1);
                    ...
                    HW_EscRead((unsigned char *) &pData[i], 0x0508, u16BytesToCopy);
                }
            }
            ...
            else
            {
                ...
                if(RetryCnt > 0)
                {
                    ...
                    int i32TimeoutTicks = (int)(1 * 10);
 
                    unsigned int u16CurTimer = 0;
                    unsigned int u16LastTimer = HAL_GetTick();
                    unsigned int Delta = 0;

                     
                    while(i32TimeoutTicks > 0)
                    {
                        u16CurTimer = HAL_GetTick();
                        ...
                        else
                        {
                            ...
                            Delta = (0xFFFFFFFFU - u16LastTimer) + u16CurTimer;
                        }
...
unsigned short ESC_EepromWriteCRC(void)
{
    unsigned short u16Return = 0x0001;
    unsigned short EscCfgData[8];
    unsigned short u16Crc = 0x00FF;
    unsigned short i,j;

    
    u16Return = ESC_EepromAccess(0,7,(unsigned short *)EscCfgData,0);
    if(u16Return == 0)
    {
        unsigned char *pData = (unsigned char *)EscCfgData;
        ...
        u16Return = ESC_EepromAccess(7,1,&u16Crc,1);
        if(u16Return == 0)
        {
            u16Return =  0x0020;
        }
