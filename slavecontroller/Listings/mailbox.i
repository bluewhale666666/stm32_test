...
typedef struct
{
 ...
 unsigned char                    bReceiveMbxIsLocked;  
 unsigned char                    bSendMbxIsFull;  
 unsigned char                    bMbxRunning;  
 unsigned char                    bMbxRepeatToggle;  
 unsigned short                  u16SendMbxSize;  
 unsigned short                  u16ReceiveMbxSize;  
 unsigned short                  u16EscAddrReceiveMbx;  
 unsigned short                  u16EscAddrSendMbx;  
 unsigned char                   u8MbxWriteCounter;  
 unsigned char                   u8MbxReadCounter;  
 
 unsigned char                   u8MailboxSendReqStored;  
 TMBX  *           psWriteMbx;  
 TMBX  *           psReadMbx;  
 TMBX  *           psRepeatMbx;  
 TMBX  *           psStoreMbx;  
 
 TMBXQUEUE         sMbxSendQueue;  
 TMBXQUEUE         sMbxReceiveQueue;  
   ...
   void     MBX_Init(void);
   unsigned short    MBX_StartMailboxHandler(void);
   void     MBX_StopMailboxHandler(void);
   void     MBX_MailboxWriteInd(TMBX  *pMbx);
   void     MBX_MailboxReadInd(void);
   void     MBX_MailboxRepeatReq(void);
   unsigned char    MBX_MailboxSendReq(TMBX  * pMbx, unsigned char flags);
   void     MBX_CheckAndCopyMailbox(void);
   unsigned char    MBX_CopyToSendMailbox(TMBX  *pMbx);
   void     MBX_Main(void);


 
#line 148 "etherCAT\\mailbox.c"
...
#line 155 "etherCAT\\mailbox.c"
 
#line 1 "etherCAT\\ecatfoe.h"
...
#line 57 "etherCAT\\ecatfoe.h"




 
#line 69 "etherCAT\\ecatfoe.h"
...
typedef struct  
{
    ...
    union 
    {
        unsigned short        Password[2];  
        unsigned short        PacketNo[2];  
...
typedef struct 
{
    ...
    unsigned short              Data[(((0x0100 - 6))-(sizeof(TFOEHEADER))) >> 1];  
}
TFOEMBX;
...
extern    unsigned int                                 u32PacketNo;  
extern    unsigned int                                 u32FileOffset;   
extern    unsigned int                                 u32LastFileOffset;  
extern    unsigned short                                 u16FileAccessState;  
extern    TMBX  *                          pFoeSendStored; 



 

extern   unsigned short                                  u16FoeMaxSendBlockSize;   
...
extern    void     FOE_Init(void);
extern    unsigned char FOE_ServiceInd(TFOEMBX  * pFoeInd);
extern    void     FOE_ContinueInd(TMBX  * pMbx);


 
#line 157 "etherCAT\\mailbox.c"
...
#line 158 "etherCAT\\mailbox.c"
    ...
    unsigned char bNoMbxMemoryAvailable;  
...
unsigned char PutInMbxQueue(TMBX  * pMbx, TMBXQUEUE  * pQueue)
{
    unsigned short lastInQueue;
    ;
...
TMBX  * GetOutOfMbxQueue(TMBXQUEUE  * pQueue)
{
    TMBX  * pMbx;
    ;
    ...
    if (pQueue->firstInQueue != pQueue->lastInQueue)
    {
        ...
        unsigned short firstInQueue = pQueue->firstInQueue;
...
unsigned short MBX_StartMailboxHandler(void)
{
    ...
    TSYNCMAN  * pSyncMan = (TSYNCMAN  *)GetSyncMan(0);
    ...
    pSyncMan =(TSYNCMAN  *) GetSyncMan(1);
    ...
    u16FoeMaxSendBlockSize = (u16SendMbxSize - sizeof(TFOEHEADER) - 6);

     
    EnableSyncManChannel(0);
     
    EnableSyncManChannel(1);

        psWriteMbx = (TMBX  *) malloc((u16ReceiveMbxSize));
        if(psWriteMbx == 0)
        {
            bNoMbxMemoryAvailable = 1;

            
            psWriteMbx = (TMBX  *) malloc((10));  
            if(psWriteMbx == 0)
            {
                result = 0x0002;
            }
...
void MBX_StopMailboxHandler(void)
{
    ...
    if (psStoreMbx != 0 && psStoreMbx != psRepeatMbx)
    {
        free((psStoreMbx));
    }

    if (psReadMbx != 0 && psReadMbx != psRepeatMbx && psReadMbx != psStoreMbx)
    {
        free((psReadMbx));
    }
...
unsigned char MailboxServiceInd(TMBX  *pMbx)
{
    unsigned char result;

     
    if(bBootMode == 1 && (4 != ((pMbx->MbxHeader.Flags[0] & 0x0F00) >> 8 )))
    {
        return 0x02;
    }

    switch ( (pMbx->MbxHeader.Flags[0] & 0x0F00) >> 8 )
    {
 
    case 3:
         
        result = COE_ServiceInd((TCOEMBX  *) pMbx);
        break;
    ...
    case 4:
        ...
        result = FOE_ServiceInd((TFOEMBX  *) pMbx);
        break;
...
void MBX_MailboxWriteInd(TMBX  *pMbx)
{
    ...
    unsigned char mbxCounter = pMbx->MbxHeader.Flags[0] >> 12;
    unsigned short MbxLen = (pMbx->MbxHeader . Length);

    if(MbxLen > 0x0100)
    {
         
        pMbx->MbxHeader.Length     = 4;
        pMbx->MbxHeader.Flags[0]        &= ~0x0F00;
        pMbx->Data[0]                        = (0x01);
        pMbx->Data[1]                        = (0x08);
...
void MBX_MailboxReadInd(void)
{
    ...
    {
        TMBX * pMbx = GetOutOfMbxQueue(&sMbxSendQueue);
      ...
      if ( u8MailboxSendReqStored )
    {
        ...
        if ( u8MailboxSendReqStored & ((unsigned char) 0x0001) )
        {
             
            EMCY_ContinueInd(psWriteMbx);
            if (EMCY_IsQueueEmpty())
            {
                u8MailboxSendReqStored &= ~((unsigned char) 0x0001);
            }
        }
        else
        if ( u8MailboxSendReqStored & ((unsigned char) 0x0002) )
        {
           unsigned char result = 0;
             
            u8MailboxSendReqStored &= ~((unsigned char) 0x0002);
        ...
        if ( u8MailboxSendReqStored & ((unsigned char) 0x0040) )
        {
             
            u8MailboxSendReqStored &= ~((unsigned char) 0x0040);
...
void MBX_MailboxRepeatReq(void)
{
    if (psRepeatMbx)
    {
       ...
       if (bSendMbxIsFull && psStoreMbx == 0)
        {
...
unsigned char MBX_MailboxSendReq( TMBX  * pMbx, unsigned char flags )
{
    ...
    if ( (nAlStatus & ((unsigned char) 0x0F)) == ((unsigned char) 0x01) )
    {
        return 0xF0;
    }
    ...
    pMbx->MbxHeader.Flags[0] |= u8MbxReadCounter << 12;
    ...
    if ( flags & ((unsigned char) 0x0080) )
    {
        

 
        u8MailboxSendReqStored |= (flags & ((unsigned char) ~((unsigned char) 0x0080)));
    }
...
void MBX_CheckAndCopyMailbox( void )
{
    unsigned short mbxLen;

 
     
    HW_EscRead(((unsigned char *)&(mbxLen)),((unsigned short)(u16EscAddrReceiveMbx)),2);
    
     
    mbxLen = (mbxLen);

    if(bNoMbxMemoryAvailable == 1)
    {
         
        TMBX  *pMbx = (TMBX  *) malloc((10));  
    ...
    else
    {
        ...
        HW_EscRead(((unsigned char *)((unsigned char *) psWriteMbx)),((unsigned short)(u16EscAddrReceiveMbx)),(mbxLen));
        ...
        volatile unsigned short smstate = 0x00;
        HW_EscRead(((unsigned char *)&(smstate)),((unsigned short)(0x0804)),2);
        smstate = (smstate);
        ...
        if(smstate & 0x0800)
        {
            ...
            u16dummy = 0;
            HW_EscRead(((unsigned char *)&(u16dummy)),((unsigned short)((u16EscAddrReceiveMbx + u16ReceiveMbxSize - 2))),2);
...
unsigned char MBX_CopyToSendMailbox( TMBX  *pMbx )
{
    ...
    else
    {
        ...
        unsigned short mbxSize = pMbx->MbxHeader.Length;
 
 
         
        {
            unsigned short LastUsedAddr = u16EscAddrSendMbx + mbxSize + 6;
            unsigned short LastAddrToReset = (u16EscAddrSendMbx + u16SendMbxSize);
            ...
            while (LastUsedAddr < LastAddrToReset)  
            {
                HW_EscWrite(((unsigned char *)&(u16dummy)),((unsigned short)(LastUsedAddr)),2);
        ...
        HW_EscWrite(((unsigned char *)((unsigned char *)pMbx)),((unsigned short)(u16EscAddrSendMbx)),((mbxSize + 6)));
        ...
        volatile unsigned short smstate = 0x00;
        HW_EscRead(((unsigned char *)&(smstate)),((unsigned short)((0x0804 + 8))),2);
        smstate = (smstate);
        ...
        if(!(smstate & 0x0800))
        {
            ...
            unsigned short BytesLeft = u16SendMbxSize - (mbxSize + 6);
            ...
            if(BytesLeft < 2)
            {
                ...
                unsigned int LastDataAddress = ((mbxSize + 6)/2);
            
                 
                memcpy((unsigned short *)&u16dummy,(((unsigned short *)pMbx) + LastDataAddress),(2 - BytesLeft));
            }

            HW_EscWrite(((unsigned char *)&(u16dummy)),((unsigned short)((u16EscAddrSendMbx + u16SendMbxSize - 2))),2);
...
void MBX_Main(void)
{
    ...
    {
        TEMCYMESSAGE  *pEmcy = GetOutOfSendEmcyQueue();
