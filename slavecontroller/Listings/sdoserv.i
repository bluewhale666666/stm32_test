...
#line 584 "etherCAT\\sdoserv.h"
 ...
 unsigned char         u8PendingSdo;  
 unsigned char          bStoreCompleteAccess;  
 unsigned char         u8StoreSubindex;  
 unsigned short        u16StoreIndex;  
 unsigned int        u32StoreDataSize;  
 unsigned short  *pStoreData;  
 unsigned char (* pSdoPendFunc)( unsigned short Index, unsigned char Subindex, unsigned int Size, unsigned short  * pData, unsigned char bCompleteAccess );  
 


 unsigned char                      aSdoInfoHeader[(((((( (6) + (2) + (4) ))+(2)))+3) >> 2)];  
 ...
 unsigned short  *                 pSdoSegData;  
 unsigned short                          nSdoInfoFragmentsLeft;  
    ...
    unsigned char SDOS_SdoInfoInd(TSDOINFORMATION  *pSdoInfoInd);
    unsigned char SDOS_SdoInd(TINITSDOMBX  *pSdoInd);

    void  SDOS_SdoRes(unsigned char abort, unsigned int objLength, unsigned short  *pData);
...
unsigned short                             nSdoInfoIndex;
const TOBJECT  *         pSdoInfoObjEntry;

TINITSDOMBX  *                pSdoResStored;
unsigned char                                bSdoInWork = 0;

unsigned char                             nSdoSegService;
unsigned char                             bSdoSegFollows;
unsigned char                             bSdoSegAccess;
unsigned short                            nSdoSegIndex;
unsigned char                             nSdoSegSubindex;
unsigned int                            nSdoSegBytesToHandle;
unsigned char                             bSdoSegLastToggle;
unsigned int                            nSdoSegCompleteSize;
const TOBJECT  *         pSdoSegObjEntry;
...
static unsigned char SdoDownloadSegmentInd(TDOWNLOADSDOSEGREQMBX  * pSdoInd);
static unsigned char SdoUploadSegmentInd(TUPLOADSDOSEGREQMBX  * pSdoInd);
...
static unsigned char SdoDownloadSegmentInd(TDOWNLOADSDOSEGREQMBX  * pSdoInd)
{
	...
	if ((pSdoInd->SdoHeader . SegHeader & ((unsigned short) 0x10)) == bSdoSegLastToggle)
	{
	...
	else
	{
		...
		unsigned short maxData = u16ReceiveMbxSize - 6 - ( 2 + 1 );
		 
		bSdoSegLastToggle = (pSdoInd->SdoHeader . SegHeader & ((unsigned short) 0x10));
		...
		if (pSdoSegData)
		{
			...
			if (pSdoInd->SdoHeader.SegHeader & ((unsigned short) 0x01))
			{
				
 
				if (bytesToSave <= maxData)
				{
					unsigned short mbxSize = (pSdoInd->MbxHeader . Length);

					


 
					if (((bytesToSave <= (unsigned int)(mbxSize - ( 2 + 1 )))
						&& (bytesToSave == ((unsigned short)(((unsigned short) 7) - ((pSdoInd->SdoHeader.SegHeader & ((unsigned short) 0x0E)) >> ((unsigned short) 1)))))
						)
						|| ((bytesToSave > ((unsigned short) 7))
							&& (bytesToSave == (unsigned int)(mbxSize - ( 2 + 1 )))
							))
					{
			...
			if (abort == 0)
			{
				...
				if ((nSdoSegBytesToHandle & 0x1) == 0x01)
				{
					...
					pSdoSegData[(nSdoSegBytesToHandle >> 1)] &= ~((unsigned short) 0xFF00);
					pSdoSegData[(nSdoSegBytesToHandle >> 1)] |= (pSdoInd->SdoHeader.SegHeader & ((unsigned short) 0xFF00));
					
					memcpy(&pSdoSegData[(nSdoSegBytesToHandle >> 1) + 1], pSdoInd->SdoHeader.Data, bytesToSave - 1);
				}
				else
				{
					
					
					pSdoSegData[(nSdoSegBytesToHandle >> 1)] = ((pSdoInd->SdoHeader.SegHeader >> ((unsigned short) 8)) &~((unsigned short) 0xFF00)) | ((pSdoInd->SdoHeader.Data[0] << 8) & ((unsigned short) 0xFF00));

					
					for (i = 1; i < ((bytesToSave + 1) >> 1); i++)
					{
						pSdoSegData[(nSdoSegBytesToHandle >> 1) + i] = ((pSdoInd->SdoHeader.Data[i - 1] >> ((unsigned short) 8)) & ~((unsigned short) 0xFF00)) | ((pSdoInd->SdoHeader.Data[i] << 8) & ((unsigned short) 0xFF00));
				...
				if (bSdoSegFollows == 0)
				{
					...
					abort = OBJ_Write(nSdoSegIndex, nSdoSegSubindex, nSdoSegCompleteSize, pSdoSegObjEntry, (unsigned short  *) pSdoSegData, bSdoSegAccess);
					if (abort == 0xFF)
					{
						...
						bSdoInWork = 1;
						pSdoResStored = (TINITSDOMBX  *) pSdoInd;
					...
					else
					{
						...
						free(((unsigned short *) pSdoSegData));
						pSdoSegData = 0;
					}
				}
			}
	...
	if (abort == 0)
	{
		...
		pSdoInd->MbxHeader.Length = ( (( 2 + 1 )) + (((unsigned short) 7)) );
		pSdoInd->CoeHeader &= ~0xF000;
		pSdoInd->CoeHeader |= ((unsigned short)0x03) << 12;
		 
		if (bSdoSegLastToggle)
		{
			pSdoInd->SdoHeader.SegHeader = (((unsigned char) (0x01 << (((unsigned char) 5)))) | ((unsigned short) 0x10));
		}
		else
		{
			pSdoInd->SdoHeader.SegHeader = (((unsigned char) (0x01 << (((unsigned char) 5)))));
		}
...
{
	unsigned char abort = 0;
	TUPLOADSDOSEGRESMBX  * pSdoSegRes = (TUPLOADSDOSEGRESMBX  *)pSdoInd;

	if ((pSdoInd->SegHeader & ((unsigned short) 0x10)) == bSdoSegLastToggle)
	{
	...
	else
	{
		...
		{
			maxData = u16SendMbxSize - 6 - ( 2 + 1 );
		}

		 
		bSdoSegLastToggle = pSdoInd->SegHeader & ((unsigned short) 0x10);
		...
		pSdoSegRes->SdoHeader.SegHeader &= ~((unsigned short) 0xFF00);
		if ((nSdoSegBytesToHandle & 0x1) == 0x01)
		{
			
			
			pSdoSegRes->SdoHeader.SegHeader |= (pSdoSegData[(nSdoSegBytesToHandle >> 1)] & ((unsigned short) 0xFF00));
			
			memcpy(pSdoSegRes->SdoHeader.Data, &pSdoSegData[(nSdoSegBytesToHandle >> 1) + 1], size - 1);
		}
		else
		{
			unsigned short i = 0;
			unsigned int nIndexOffset = nSdoSegBytesToHandle >> 1;

			
			
			pSdoSegRes->SdoHeader.SegHeader |= ((pSdoSegData[(nSdoSegBytesToHandle >> 1)] << ((unsigned short) 8)) & ((unsigned short) 0xFF00));
			

			for (i = 0; i < (size >> 1); i++)
			{
				pSdoSegRes->SdoHeader.Data[i] = ((pSdoSegData[i + nIndexOffset] & ((unsigned short) 0xFF00)) >> 8) | ((pSdoSegData[i + nIndexOffset + 1] & ~((unsigned short) 0xFF00)) << 8);
		...
		pSdoSegRes->CoeHeader &= ~0xF000;
		pSdoSegRes->CoeHeader |= ((unsigned short)0x03) << 12;

		 
		pSdoSegRes->SdoHeader.SegHeader &= ~((unsigned short) 0xFF);
		if (bSdoSegFollows)
		{
			pSdoSegRes->SdoHeader.SegHeader |= (((unsigned char) (0x00 << (((unsigned char) 5)))) | bSdoSegLastToggle);
		}
		else
		{
			pSdoSegRes->SdoHeader.SegHeader |= (((unsigned char) (0x00 << (((unsigned char) 5)))) | bSdoSegLastToggle | ((unsigned short) 0x01));
		}

				
				 
		if (size < ((unsigned short) 7))
		{
			
			pSdoSegRes->MbxHeader.Length = ( (( 2 + 1 )) + (((unsigned short) 7)) );
			pSdoSegRes->SdoHeader.SegHeader |= (((unsigned short) 7) - size) << ((unsigned short) 1);
		}
		else
		{
			pSdoSegRes->MbxHeader.Length = ((unsigned short)size) + ( 2 + 1 );
		}
...
void SdoRes(unsigned char abort, unsigned char command, unsigned char completeAccess, unsigned short dataSize, unsigned int objLength, TINITSDOMBX  *pSdoRes)
{
	 
	if ((command != ((unsigned char) (0x03 << (((unsigned char) 5))))) && (command != ((unsigned char) (0x00 << (((unsigned char) 5))))))
	{
 
		pSdoRes->SdoHeader.Sdo[0] &= 0xFF00;
 
	}
	if (abort == 0)
	{
		 
		pSdoRes->CoeHeader &= ~0xF000;
		pSdoRes->CoeHeader |= ((unsigned short)0x03) << 12;
		if (command == ((unsigned char) (0x02 << (((unsigned char) 5)))))
		{
			
			if ((objLength <= 4) && (objLength > 0))
			{
				 
				pSdoRes->MbxHeader.Length = ( 2 + 4 + (4) );
				pSdoRes->SdoHeader.Sdo[0] |= (((unsigned char) 0x01) | ((unsigned char) 0x02) | completeAccess | ((4 - ((unsigned char)objLength)) << ((unsigned char) 2)) | ((unsigned char) (0x02 << (((unsigned char) 5)))));
			...
			else
			{
				...
				{
					pSdoRes->MbxHeader.Length = ( 2 + 4 + 4 ) + dataSize;
				}
				else
				{
					pSdoRes->MbxHeader.Length = ( 2 + 4 + 4 ) + ((unsigned short)objLength);
				}
				((TINITSDOUPLOADNORMRESMBX  *) pSdoRes)->CompleteSize[0] = ((unsigned short)objLength);
				((TINITSDOUPLOADNORMRESMBX  *) pSdoRes)->CompleteSize[1] = ((unsigned short)(objLength >> 16));
				pSdoRes->SdoHeader.Sdo[0] |= (((unsigned char) 0x01) | completeAccess | ((unsigned char) (0x02 << (((unsigned char) 5)))));
		...
		else if (command == ((unsigned char) (0x00 << (((unsigned char) 5)))))
		{
			 
			pSdoRes->MbxHeader.Length = ( 2 + 4 + 4 );
			pSdoRes->SdoHeader.Sdo[0] |= (((unsigned char) (0x01 << (((unsigned char) 5)))));
 
		}
		else if (command != ((unsigned char) (0x03 << (((unsigned char) 5)))))
		{
			 
			pSdoRes->MbxHeader.Length = ( 2 + 4 + 4 );
			pSdoRes->SdoHeader.Sdo[0] |= (((unsigned char) (0x03 << (((unsigned char) 5)))));
	...
	else
	{
		...
		pSdoRes->MbxHeader.Length = (sizeof(TABORTSDOTRANSFERREQMBX) - sizeof(TMBXHEADER));
		pSdoRes->CoeHeader &= ~0xF000;
		pSdoRes->CoeHeader |= ((unsigned short)0x02) << 12;
		pSdoRes->SdoHeader.Sdo[0] |= (((unsigned char) (((unsigned char) 0x04) << (((unsigned char) 5)))));
 

 
		((TABORTSDOTRANSFERREQMBX  *) pSdoRes)->AbortCode = (cAbortCode[abort]);
	}

	
	
	if (MBX_MailboxSendReq((TMBX  *) pSdoRes, ((unsigned char) 0x0002)) != 0)
	{
		 
		pCoeSendStored = (TMBX  *) pSdoRes;
	}
}
...
{
	unsigned char abort = 0;
	unsigned char sdoHeader = (pSdoInd->SdoHeader.Sdo[0] & 0xFF) >> 0;
 
	 
	unsigned char command = (sdoHeader & ((unsigned char) 0xE0));
	...
	unsigned char bCompleteAccess = 0;
	...
	if (sdoHeader & ((unsigned char) 0x10))
	{
		bCompleteAccess = 1;
	}
	...
	switch (command)
	{
	case ((unsigned char) (0x01 << (((unsigned char) 5)))):
	case ((unsigned char) (0x02 << (((unsigned char) 5)))):
		 
		index = pSdoInd->SdoHeader.Sdo[1] & 0xFF;
 
		index <<= 8;
		index += (pSdoInd->SdoHeader.Sdo[0] >> 8) & 0x00FF;
		 
		subindex = (pSdoInd->SdoHeader.Sdo[1] >> 8) & 0xFF;
		...
		if (pObjEntry)
		{
			...
			unsigned char bTransferType = 0;
			 
			unsigned short  * pData = 0;
			unsigned char segTransfer = 0;

			{
				...
				{
					dataSize = objLength = OBJ_GetObjectLength(index, subindex, pObjEntry, (unsigned char)(sdoHeader & ((unsigned char) 0x10)));
				}
				...
				if (abort == 0)
				{
					if (command == ((unsigned char) (0x02 << (((unsigned char) 5)))))
					{
						...
						if ((objLength <= 4) && objLength != 0)
						{
							...
							pData = (unsigned short  *) ((TINITSDOUPLOADEXPRESMBX  *) pSdoInd)->Data;
						...
						else
						{
							...
							dataSize = u16SendMbxSize - 6 - ( 2 + 4 + 4 );
							if (dataSize < objLength)
							{
							...
							else
							{
								...
								pData = (unsigned short  *) ((TINITSDOUPLOADNORMRESMBX  *) pSdoInd)->Data;
							}
						}
					}
					else
					{
						...
						bTransferType = sdoHeader & ((unsigned char) 0x02);
					}
				}
			}

			if ((abort == 0) && (command == ((unsigned char) (0x01 << (((unsigned char) 5))))))
			{
				...
				if (bTransferType)
				{
					...
					dataSize = 4 - ((sdoHeader & ((unsigned char) 0x0C)) >> ((unsigned char) 2));
					 
					pData = (unsigned short  *) &pSdoInd[1];
				}
				else
				{
					...
					unsigned int downloadSize = ((unsigned int)((((TINITSDODOWNLOADNORMREQMBX *) pSdoInd)->CompleteSize[1])) << 16) + ((((TINITSDODOWNLOADNORMREQMBX *) pSdoInd)->CompleteSize[0]));

					 
					if ((6 + ( 2 + 4 + (4) ) + downloadSize) > u16ReceiveMbxSize)
					{
						if (mbxSize != (u16ReceiveMbxSize - 6))
						{
							return 0x08;
						}
					}
					...
					pData = (unsigned short  *) ((TINITSDODOWNLOADNORMREQMBX  *) pSdoInd)->Data;
					
 
					dataSize = downloadSize;
					if (dataSize > (unsigned int)(mbxSize - ( 2 + 4 + 4 )))
					{
			...
			if (abort == 0)
			{
				if (segTransfer)
				{
					...
					pSdoSegData = (unsigned short  *) malloc((((((nSdoSegCompleteSize)+1)>>1)<<1)));
					...
					else
					{
						if (command == ((unsigned char) (0x02 << (((unsigned char) 5)))))
						{
							...
							abort = OBJ_Read(index, subindex, objLength, pObjEntry, (unsigned short  *) pSdoSegData, bCompleteAccess);
							if (abort == 0)
							{
								memcpy((unsigned short *)((TINITSDOUPLOADNORMRESMBX  *) pSdoInd)->Data, pSdoSegData, dataSize);
								nSdoSegService = ((unsigned char) (0x03 << (((unsigned char) 5))));
							}
							else if (abort == 0xFF)
							{
								...
								pSdoResStored->SdoHeader.Sdo[0] &= ~0xFF;
								pSdoResStored->SdoHeader.Sdo[0] |= (sdoHeader & (((unsigned char) 0x10) | ((unsigned char) 0xE0)));
								nSdoSegService = ((unsigned char) (0x03 << (((unsigned char) 5))));
								return 0;
							}
						}
						else
						{
							...
							memcpy(pSdoSegData, (unsigned short *)((TINITSDODOWNLOADNORMREQMBX  *) pSdoInd)->Data, mbxSize - ( 2 + 4 + 4 ));
							nSdoSegService = ((unsigned char) (0x00 << (((unsigned char) 5))));
							dataSize = (mbxSize - ( 2 + 4 + 4 ));
						}
	...
	case ((unsigned char) (0x00 << (((unsigned char) 5)))):
	case ((unsigned char) (0x03 << (((unsigned char) 5)))):
		if (command == nSdoSegService)
		{
			if (command == ((unsigned char) (0x00 << (((unsigned char) 5)))))
			{
				abort = SdoDownloadSegmentInd((TDOWNLOADSDOSEGREQMBX  *) pSdoInd);
			}
			else
			{
				abort = SdoUploadSegmentInd((TUPLOADSDOSEGREQMBX  *) pSdoInd);
			}
		}
		else
		{
	...
	if (abort != 0xFF)
	{
		...
		SdoRes(abort, command, (unsigned char)(sdoHeader & ((unsigned char) 0x10)), (unsigned short)dataSize, objLength, pSdoInd);
	}
...
void SDOS_SdoRes(unsigned char abort, unsigned int objLength, unsigned short  *pData)
{
	...
	if (bSdoInWork)
	{
		...
		unsigned char command = pSdoResStored->SdoHeader.Sdo[0] & ((unsigned char) 0xE0);
		unsigned char completeAccess = pSdoResStored->SdoHeader.Sdo[0] & ((unsigned char) 0x10);
		...
		if (command == ((unsigned char) (0x02 << (((unsigned char) 5)))))
		{
			...
			if (dataSize < objLength)
			{
				...
				nSdoSegCompleteSize = objLength;
				nSdoSegService = ((unsigned char) (0x03 << (((unsigned char) 5))));
				pSdoSegData = (unsigned short  *) pData;
				 
				memcpy((unsigned short *)((TINITSDOUPLOADNORMRESMBX  *) pSdoResStored)->Data, pData, dataSize);
				nSdoSegBytesToHandle = dataSize;
			}
			else
				if ((objLength <= 4) && (objLength > 0))
				{
					...
					if (pData != ((TINITSDOUPLOADEXPRESMBX  *) pSdoResStored)->Data)
					{
						 
						memcpy((unsigned short *)((TINITSDOUPLOADEXPRESMBX  *) pSdoResStored)->Data, pData, objLength);
					}
				}
				else
				{
					 
					if (pData != ((TINITSDOUPLOADNORMRESMBX  *) pSdoResStored)->Data)
					{
						 
						memcpy((unsigned short *)((TINITSDOUPLOADNORMRESMBX  *) pSdoResStored)->Data, pData, objLength);
					}
				}
		}
...
void  SDOS_ClearPendingResponse(void)
{
	if ((bSdoInWork == 1) && (pSdoResStored != 0) && (((TMBX  *)pSdoResStored) != psRepeatMbx) && (((TMBX  *)pSdoResStored) != psStoreMbx))
	{
		free((pSdoResStored));
		pSdoResStored = 0;
	}
...
unsigned char SDOS_SdoInfoInd(TSDOINFORMATION  *pSdoInfoInd)
{
	...
	unsigned char opCode = (unsigned char)((pSdoInfoInd->SdoHeader.InfoHead & 0x007F) >> 0);
	const TOBJECT  * pObjEntry;
	unsigned short index;
	unsigned char flags = ((unsigned char) 0x0002);

	
 
	if (opCode == 0x05)
	{
		if (pSdoInfoInd->MbxHeader.Length < ( (2) + (4) + (4) ))
		{
			return 0x06;
		}
	}
	...
	{
		if (pSdoInfoInd->MbxHeader.Length < (( 2) + (4) + (2) ))
		{
			return 0x06;
		}
	}
	...
	case 0x01:
		...
		if ((pSdoInfoInd->SdoHeader . Data . List . ListType) <= 5)
		{
			unsigned short size = 0;
			 
			unsigned char listType = (unsigned char)(pSdoInfoInd->SdoHeader . Data . List . ListType);

			


 
			memcpy(aSdoInfoHeader, pSdoInfoInd, ((( (6) + (2) + (4) ))+(2)));
			if (listType-- == 0)
			{
				...
				for (i = 0; i < 5; i++)
				{
					unsigned short n = OBJ_GetNoOfObjects(i);

					 
					((unsigned short  *) &pSdoInfoInd->CoeHeader)[((( 2) + (4) + (2) ) >> 1) + i] = (n);
				}
			...
			else
			{
				...
				if (nSdoInfoFragmentsLeft)
				{
					...
					{
						size = u16SendMbxSize - ( (2) + (4 )) - 6;
					}
					 
					pData = &((unsigned short  *) &pSdoInfoInd->CoeHeader)[( (2) + (4 )) >> 1];
				...
				else
				{
					...
					{
						size = u16SendMbxSize - (( 2) + (4) + (2) ) - 6;
					}
					 
					pData = &((unsigned short  *) &pSdoInfoInd->CoeHeader)[(( 2) + (4) + (2) ) >> 1];
					...
					if ((n << 1) > size)
					{
						...
						unsigned short Fragsize = size + 2;
			...
			if (abort == 0)
			{
				pSdoInfoInd->SdoHeader.InfoHead &= ~0x007F;
				pSdoInfoInd->SdoHeader.InfoHead |= (unsigned short)(0x02 << 0);
 
				 
				pSdoInfoInd->SdoHeader.FragmentsLeft = (nSdoInfoFragmentsLeft);

				if (nSdoInfoFragmentsLeft)
				{
					
 
					pSdoInfoInd->SdoHeader.InfoHead &= ~0x0080;
					pSdoInfoInd->SdoHeader.InfoHead |= (unsigned short)(0x80 << 0);
					


 
					flags = ((unsigned char) 0x0080) | ((unsigned char) 0x0002);
				}
			}
		}
		break;
	...
	case 0x05:
		...
		index = (pSdoInfoInd->SdoHeader . Data . Obj . Index);
		...
		else
		{
			...
			if (pObjEntry)
			{
				...
				if (opCode == 0x03)
				{
					...
					memcpy(&pSdoInfoInd->SdoHeader.Data.Obj.Res, OBJ_GetObjDesc(pObjEntry), sizeof(TSDOINFOOBJDESC));
					...
					size = OBJ_GetDesc(index, 0, pObjEntry, 0) + ( (2) + (4) + (sizeof(TSDOINFOOBJ)) );

					if (size > (u16SendMbxSize - 6))
					{
						
 
						size = ( (2) + (4) + (sizeof(TSDOINFOOBJ)) );
					}
					else
					{
						 
						size = OBJ_GetDesc(index, 0, pObjEntry, ((unsigned short  *) &(&pSdoInfoInd->SdoHeader.Data.Obj.Res)[1])) + ( (2) + (4) + (sizeof(TSDOINFOOBJ)) );
					}
				}
				else
				{
					...
					unsigned char subindex = (unsigned char)((pSdoInfoInd->SdoHeader.Data.Entry.Info & 0x00FF) >> 0);

					 
					unsigned char maxSubindex = (OBJ_GetObjDesc(pObjEntry)->ObjFlags & 0x00FF) >> 0;
					...
					if (subindex <= maxSubindex)
					{
						...
						memcpy(&pSdoInfoInd->SdoHeader.Data.Entry.Res, OBJ_GetEntryDesc(pObjEntry, subindex), sizeof(TSDOINFOENTRYDESC));

						 
						pSdoInfoInd->SdoHeader.Data.Entry.Info &= ~0xFF00;
						ObjectFlags = OBJ_GetObjDesc(pObjEntry)->ObjFlags;
						ObjectFlags = (ObjectFlags & 0x0F00) >> 8;

						if (((ObjectFlags == 0x08) || (ObjectFlags == 0x09)) && (subindex == 0))
						{
							memcpy(((unsigned short  *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1]), aSubindexDesc, sizeof(aSubindexDesc));
							size = 12 + ( (2) + (4 )) + sizeof(TSDOINFOENTRY); 
						}
						else
						{
							...
							size = OBJ_GetDesc(index, subindex, pObjEntry, 0) + ( (2) + (4 )) + sizeof(TSDOINFOENTRY);
							if (size > (u16SendMbxSize - 6))
							{
								
 
								size = ( (2) + (4 )) + sizeof(TSDOINFOENTRY);
							}
							else
							{
								 
								size = OBJ_GetDesc(index, subindex, pObjEntry, ((unsigned short  *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1])) + ( (2) + (4 )) + sizeof(TSDOINFOENTRY);
							}
						}
					}
					else
				...
				{
					{
						...
						pSdoInfoInd->SdoHeader.InfoHead &= ~0x007F;
						pSdoInfoInd->SdoHeader.InfoHead |= (unsigned short)((opCode + 1) << 0);
					}
				}
			}
			else
	...
	if (abort)
	{
		...
		pSdoInfoInd->MbxHeader.Length = ( (2) + (4) + (sizeof(TSDOINFOERROR)) );

		pSdoInfoInd->SdoHeader.InfoHead &= ~0x007F;
		pSdoInfoInd->SdoHeader.InfoHead |= (unsigned short)((0x07) << 0);

		pSdoInfoInd->SdoHeader.FragmentsLeft = 0;
 
		pSdoInfoInd->SdoHeader.Data.Error.ErrorCode = (cAbortCode[abort]);

		nSdoInfoFragmentsLeft = 0;
	}

	if (MBX_MailboxSendReq((TMBX  *) pSdoInfoInd, flags) != 0)
	{
		


 
		pCoeSendStored = (TMBX  *) pSdoInfoInd;
	}
