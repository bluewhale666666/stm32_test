...
;
    ...
    unsigned int  OBJ_GetObjectLength( unsigned short index, unsigned char subindex, const TOBJECT  * pObjEntry, unsigned char bCompleteAccess);
    ...
    unsigned char   OBJ_Read(unsigned short index, unsigned char subindex, unsigned int objSize, const TOBJECT  * pObjEntry, unsigned short  * pData, unsigned char bCompleteAccess);
    unsigned char   OBJ_Write(unsigned short index, unsigned char subindex, unsigned int dataSize, const TOBJECT  * pObjEntry, unsigned short  * pData, unsigned char bCompleteAccess);
    void    COE_WriteBackupEntry(unsigned char subindex, const TOBJECT  * pObjEntry);


 
#line 138 "etherCAT\\objdef.c"
...
unsigned char OBJ_Read( unsigned short index, unsigned char subindex, unsigned int objSize, const TOBJECT  * pObjEntry, unsigned short  * pData, unsigned char bCompleteAccess )
{
    ...
    if ( bCompleteAccess )
    {
        ...
        else if ( index < 0x1000 && subindex != 0 )
        {
            ...
            if((((unsigned char)pVarPtr) & 0x1) == 0x1)
            {
                ...
                pVarPtr = (unsigned short  *)(((unsigned char)pVarPtr)& ~(unsigned char)0x1);
...
unsigned char OBJ_Write( unsigned short index, unsigned char subindex, unsigned int dataSize, const TOBJECT  * pObjEntry, unsigned short  * pData, unsigned char bCompleteAccess )
{
    ...
    if ( pObjEntry->Write != 0 )
    {
        ...
        if ((result == 0) && (pAPPL_CoeWriteInd != 0))
        {
