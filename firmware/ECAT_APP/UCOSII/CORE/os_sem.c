...
*********************************************************************************************************
*/
//当一个任务请求信号量时,如果希望在信号量无效时准许任务不进入等待状态而继续运行
//则不调用OSSemPend(),而调用函数OSSemAccept()来请求信号量。
...
OS_EVENT  *OSSemCreate (INT16U cnt)
{
    ...
    if (pevent != (OS_EVENT *)0) {                         /* Get an event control block               */
        ...
        pevent->OSEventCnt     = cnt;                      /*设置计数器初值 Set semaphore value                      */
        pevent->OSEventPtr     = (void *)0;                /*设置空指针Unlink from ECB free list                */
#if OS_EVENT_NAME_EN > 0u
        pevent->OSEventName    = (INT8U *)(void *)"?";
#endif
        OS_EventWaitListInit(pevent);                      /*初始化事件控制块Initialize to 'nobody waiting' on sem.   */
    }
    return (pevent);
}
                 ...
                 INT8U     *perr)
{
...
#endif 
    if (pevent->OSEventType != OS_EVENT_TYPE_SEM) {   /*确保该event控制块为Sem类型 Validate event block type                     */
        *perr = OS_ERR_EVENT_TYPE;
        return;
    }
    ...
    if (OSLockNesting > 0u) {                         /* 被锁后不能调用OSSemPend() See if called with scheduler locked ...       */
        *perr = OS_ERR_PEND_LOCKED;                   /* ... can't PEND when locked                    */
        return;
    }
    OS_ENTER_CRITICAL();
    if (pevent->OSEventCnt > 0u) {                    /* 当信号量大于0时,可以直接使用事件If sem. is positive, resource available ...   */
        pevent->OSEventCnt--;                         /* ... decrement semaphore only if positive.     */
        OS_EXIT_CRITICAL();
        *perr = OS_ERR_NONE;
        return;
    }
                                                      /* 当信号量等于0时,资源不能使用Otherwise, must wait until event occurs       */
    OSTCBCur->OSTCBStat     |= OS_STAT_SEM;           /* 资源不可用,进入等待状态Resource not available, pend on semaphore     */
    OSTCBCur->OSTCBStatPend  = OS_STAT_PEND_OK;       
    OSTCBCur->OSTCBDly       = timeout;               /* timeout写入到TCB的OSTCBDly中  Store pend timeout in TCB                     */
    OS_EventTaskWait(pevent);                         /* 使任务进入等待状态 Suspend task until event or timeout occurs    */
    OS_EXIT_CRITICAL();
    OS_Sched();                                       /* 调用一次任务调度器 Find next highest priority task ready         */
    OS_ENTER_CRITICAL();
...
*********************************************************************************************************
*/
//发送信号量(释放信号量)任务在访问完共享资源后,必须释放信号量,
//释放信号量也叫做发送信号量
INT8U  OSSemPost (OS_EVENT *pevent)
{
    ...
    OS_ENTER_CRITICAL();
    if (pevent->OSEventGrp != 0u) {                   /* 找出等待中的最高优先级的任务 See if any task waiting for semaphore         */
                                                      /* Ready HPT waiting on event                    */
        (void)OS_EventTaskRdy(pevent, (void *)0, OS_STAT_SEM, OS_STAT_PEND_OK);
        OS_EXIT_CRITICAL();
        OS_Sched();                                   /* 找到等待中的最高优先级的任务后调用一次任务调度器 Find HPT ready to run                         */
        return (OS_ERR_NONE);
    }
    if (pevent->OSEventCnt < 65535u) {                /* 没有等待中的任务 Make sure semaphore will not overflow         */
        pevent->OSEventCnt++;                         /* OSEventCnt加一 Increment semaphore count to register event   */
        OS_EXIT_CRITICAL();
        return (OS_ERR_NONE);
    }
...
*********************************************************************************************************
*/
//查询信号量的状态,任务可以调用此函数查询信号量当前的状态,
//任务调用此函数对信号量查询后,会把信号量中的相关信息存储到OS_SEM_DATA结构体变量中
//调用此函数前须定义一个OS_SEM_DATA结构的类型变量
//*pevent:信号量指针
//*p_sem_data: 存储信号量状态的结构 
