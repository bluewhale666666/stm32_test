    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                OS_SchedNew();
                OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];  //获取最高级别就绪任务的prio
                if (OSPrioHighRdy != OSPrioCur) {          /* 最高优先级任务不是被中断的任务No Ctx Sw if current task is highest rdy */
#if OS_TASK_PROFILE_EN > 0u
                    OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task  */
#endif
                    OSCtxSwCtr++;                          /* Keep track of the number of ctx switches */
                    OSIntCtxSw();                          /* 执行中断级人物切换 Perform interrupt level ctx switch       */
                }
            }
        }
        OS_EXIT_CRITICAL();
...
//OSTimeTick 一共做了两件事
//一:给计数器OSTime加一
//二:遍历完任务控制块链表中的所有任务控制块,把各个任务控制块中用于存放任务延时时限的OSTCBDly变量减一
//同时又不使挂起的任务进入就绪状态
...
#endif  
        ptcb = OSTCBList;                                  /*指向TCB控制块列表的第一个成员 Point at first TCB in TCB list */
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /*遍历完所有的TCB控制块 Go through all TCBs in TCB list*/
            OS_ENTER_CRITICAL();                           //关中断,进临界段
...
*/
//任务控制块初始化 
//函数的作用是将变量OSEventGrp以及任务等待表中的每一位都清0,令事件的任务等待表中不含有任何等待任务
#if (OS_EVENT_EN)
...
INT8U  OS_TCBInit (INT8U    prio,  //任务优先级,保存在OSTCBPrio中
