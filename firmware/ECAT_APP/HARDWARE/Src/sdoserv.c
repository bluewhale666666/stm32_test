            ...
            if ( pObjEntry )
            {
                ...
                else
                {
                    ...
                    if ( subindex <= maxSubindex )
                    {
                        ...
                        OBJTOMBXMEMCPY(&pSdoInfoInd->SdoHeader.Data.Entry.Res, OBJ_GetEntryDesc(pObjEntry, subindex), 6/*SIZEOF(TSDOINFOENTRYDESC)2019.4.24*/);
                        ...
                        if(((ObjectFlags == OBJCODE_ARR) || (ObjectFlags == OBJCODE_REC)) && (subindex == 0) )
                        {
                            ...
                            size = 12 + SIZEOF_SDOINFO + 10;//2019.4.24 SIZEOF(TSDOINFOENTRY); // 12: Length of "SubIndex 000"
                        }
                        else
                        {
                            ...
                            size = OBJ_GetDesc(index, subindex, pObjEntry, NULL) + SIZEOF_SDOINFO + 10/*2019.4.24SIZEOF(TSDOINFOENTRY)*/;
                            if ( size > (u16SendMbxSize - MBX_HEADER_SIZE) )
                            {
                                /* size of the object description does not fit in the mailbox,
                                the object description will be sent without the name */
                                size =  SIZEOF_SDOINFO + 10;//2019.4.24SIZEOF(TSDOINFOENTRY);
                            }
                            else
                            {
                                /* object description fits in the mailbox, get the name of the entry */
                                size = OBJ_GetDesc(index, subindex, pObjEntry, ((UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1])) + SIZEOF_SDOINFO + 10/*2019.4.24SIZEOF(TSDOINFOENTRY)*/;
                            }
                        }
                    }
                    else
